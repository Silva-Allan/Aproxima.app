// src/services/upload.ts - VERSÃO FINAL CORRIGIDA (sem erro)
...
export class UploadService {
   ...
   */
  static async uploadGestoImagem(gestoId: number, userId: string, imageUri: string): Promise<UploadResult> {
    try {
      const response = await fetch(imageUri);
      const blob = await response.blob();
      ...
      const timestamp = Date.now();
      const random = Math.random().toString(36).substring(2, 9);
      const fileName = `${userId}/gesto_${gestoId}_${timestamp}_${random}.jpg`;
      ...
      if (error) {
        console.error('Erro no upload de imagem do gesto:', error);
        return {
          success: false,
          error: error.message
        };
